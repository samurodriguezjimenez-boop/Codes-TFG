...
N_SPINS   = 12          # Número de espines (cadena 1D con PBC)
J_COUP    = 1.0         # Acoplamiento ferromagnético
h_1  = np.linspace(0.1, 0.5, 2)  # Zona paramagnética
h_2 = np.linspace(0.6, 1.5, 10)  # Zona crítica (más densa)
h_3 = np.linspace(1.7, 1.9, 2)  # Zona ferromagnética
h_4 = np.linspace(2.0, 2.0, 1)  # Zona ferromagnética fuerte
H_FIELDS = np.concatenate([h_1, h_2, h_3, h_4])  # Campo transversal a estudiar
...
for h_val in H_FIELDS:
    ...
    print(f'g={h_val:.2f} completado.')

print(f"  J/h = {H_FIELDS[7]:.2f}: m = {ED_results[H_FIELDS[7]]['m']:.4f}, "
...
# ─────────────────────────────────────────────────────────────────────────────
# 6. BUCLE PRINCIPAL: BARRE TODOS LOS VALORES DE J/h
# ─────────────────────────────────────────────────────────────────────────────

print("=" * 60)
print("Entrenando RBM para cada valor de J/h...")
print("=" * 60)

rbm_results = {}

for idx, h_val in enumerate(H_FIELDS):
    print(f"\n[{idx+1}/{len(H_FIELDS)}] J/h = {h_val:.3f}")
...
resultados = {
    "h_arr": h_arr,
    "m_rbm": m_rbm, "m_err_rbm": m_err_rbm,
    "chi_rbm": chi_rbm, "chi_err_rbm": chi_err_rbm,
    "E_rbm": E_rbm,
    "m_ed": m_ed,
    "chi_ed": chi_ed,
    "E_ed": E_ed
...
resultados = {k: v[1:] for k, v in resultados.items()}  # Omitimos el primer punto (J/h=0.1) para mejor escala

# ─── Figura 1: Observables vs J/h ───────────────────────────────────────────
...
ax.errorbar(h_arr, E_rbm, yerr=m_err_rbm, fmt="ro--", capsize=4,
            label="RBM con 3 capas ocultas", markersize=6)
ax.set_xlabel("J/h (campo transverso)", fontsize=12)
...
ax.errorbar(h_arr, m_rbm, yerr=m_err_rbm, fmt="bs--", capsize=4,
            label="RBM con 3 capas ocultas", markersize=6)
ax.axvline(x=1.0, color="gray", ls=":", alpha=0.7, label="Punto crítico (J/h≈1)")
ax.set_xlabel("J/h (campo transverso)", fontsize=12)
...
# ─── Figura 2: Convergencia durante entrenamiento (para J/h cercano al punto crítico) ───
# El punto más interesante es cerca de la transición de fase J/h ≈ 1
critical_idx  = np.argmin(np.abs(h_arr - 1.0))  # J/h más cercano al punto crítico
h_critical    = H_FIELDS[critical_idx]
history_crit  = rbm_results[h_critical]["history"]

fig, axes = plt.subplots(1, 2, figsize=(12, 5))
fig.suptitle(
    f"Convergencia del entrenamiento VMC (J/h = {h_critical:.2f}, cerca del punto crítico)",
...
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 75)
print(f"{'J/h':>6} | {'m_ED':>7} | {'m_RBM':>7} | {'χ_ED':>7} | {'χ_RBM':>7} | {'Δχ/χ':>7}")
print("-" * 75)
for h_val in H_FIELDS:
    ed  = ED_results[h_val]
